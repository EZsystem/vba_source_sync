...
' Module        : xl_mod_CostMng1
' 概要          : 「�ａvが"1"のたびに「小規模工事名」から分割し、「基本工事コード」「基本工事名」を転写
' 引数          : arr1 - 2次元配列
'============================================
Public Sub Transfer_ProcessedConstructionName_Updated(arr1 As Variant)
    ...
    For r = 2 To rowCount
        ...
        If currentNo = "" Then
            isNewGroup = True
        Else
            ' 「�ａvが"1" または 空白の後であれば、新しいグループとみなす
...
' 概要          : 行を削除した後、最後に「�ａv列を「管理番号」へ変換する
'============================================
Public Sub RemoveRowsByEmptyCode_Final(ByRef arr1 As Variant)
    ...
    colBranchCode = GetTitleColumn(arr1, "枝番工事コード")
    colNo = GetTitleColumn(arr1, "��") ' ここではまだ「�ａvという名前
    ...
    For r = 2 To rowCount
        [cite_start] ' �ｂｪ2の行、またはコードが埋まっている行を保持
