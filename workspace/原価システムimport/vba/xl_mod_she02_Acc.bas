Attribute VB_Name = "she02_Acc"
Option Explicit



'==========================================================
' Sub名: she02_Acc_update_1
' 概要: ExcelのデータをAccessの特定のテーブルに追加し、
'       空白セルをAccessのデフォルト値で補完する
' 日付: 2025/01/27
'==========================================================
Sub she02_Acc_update_1()
    ...
    tableName = ThisWorkbook.Sheets("G1_原価S直データ").Range("R2").value ' テーブル名
    ...
    For i = 2 To UBound(data, 1) ' データは7行目から開始（タイトル行は6行目）
        ' C列がブランクの場合はスキップ
        If IsEmpty(data(i, 3)) Or data(i, 3) = "" Then GoTo SkipRecord
