...
Public Sub Inport_koujiitiran_FromAccess()
    ...
    Set ws = ThisWorkbook.Sheets("原価S_基本工事")
    Set tbl = ws.ListObjects("tbl_原価S_基本工事")

    ' --- Access接続パラメータの取得 ---
    Dim accPath As String: accPath = ws.Range("C3").value
    Dim queryName As String: queryName = ws.Range("C4").value
